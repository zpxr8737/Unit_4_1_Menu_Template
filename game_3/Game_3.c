...
{ 255,   0,   3,   7,   7,   7,   7,   7,   7,   7,   3,   7,   7,   3,   3,   3 },
...
static void drawGame(void){
    ...
    for(int i = 0; i<game.targetCount;i++){
        ...
        if(game.targets[i].hitFlashEnd && HAL_GetTick() < game.targets[i].hitFlashEnd){
            LCD_Draw_Sprite_Colour_Scaled((uint16_t)(game.targets[i].pos.x-16),(uint16_t)(game.targets[i].pos.y - 16),16,16,sprite,1,2);
        }
        else{
            LCD_Draw_Sprite_Scaled((uint16_t)(game.targets[i].pos.x-16), (uint16_t)(game.targets[i].pos.y-16), 16, 16, sprite, 2);
        }
    }

    char buf[32];
    sprintf(buf,"Score: %d", game.score);
    LCD_printString(buf,10,10,15,2);
    sprintf(buf, "Lives: %d", game.lives);
    LCD_printString(buf, 140, 10, 15, 2);
    if(game.combo>1){
        sprintf(buf, "x%d COMBO!", game.combo);
        LCD_printString(buf, 80, 50, 15, 2);
    }

    // pause menu
    if(game.state==statePaused){
        LCD_Draw_Rect(20,30,200,180,0,1);
        LCD_printString("PAUSED", 75, 40, 15, 2);
        LCD_printString("Sensitivity:", 35, 80, 15, 1);
        sprintf(buf, "%.2f", game.sensitivity);
        LCD_printString(buf, 150, 80, 15, 1);
        LCD_printString("LEFT/RIGHT adjust", 35, 105, 15, 1);
        LCD_printString("Joystick Press: resume", 35, 125, 15, 1);
        LCD_printString("Button: menu", 35, 145, 15, 1);
    }
    //game over
    if(game.state==stateGameOver){
        LCD_printString("GAME OVER", 60, 30, 15, 2);
        sprintf(buf, "Score: %d", game.score);
        LCD_printString(buf, 70, 70, 15, 2);
        sprintf(buf, "High: %d", game.highScore);
        LCD_printString(buf, 50, 100, 15, 1);
        LCD_printString("Button: menu", 75, 150, 15, 1);
    }
